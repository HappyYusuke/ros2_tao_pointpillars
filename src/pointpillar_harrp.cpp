...
)
{
  ...
  // ==========================================
  // 【追加機能】 ここからフィルタリング処理を開始
  // 標準のNMS結果(nms_pred)に対して、スコア閾値でさらに選別を行います
  // ==========================================
  
  // 設定: スコア閾値と対象クラス名
  const float MIN_SCORE = 0.8f;            // 0.4未満のスコアは除外
  const std::string TARGET_CLASS = "Pedestrian"; // フィルタリング対象のクラス

  // 一時保存用ベクター
  std::vector<Bndbox> filtered_pred;
  ...
  for (const auto& box : nms_pred) {
    ...
    if (box.id >= 0 && box.id < class_names.size()) {
        current_class_name = class_names[box.id];
    }

    // 対象クラス（Pedestrianなど）の場合のみ、スコア判定を行う
    if (current_class_name == TARGET_CLASS) {
        if (box.score < MIN_SCORE) {
            keep = false; // スコアが低いので捨てる
        }
    }
    
    // 条件を満たしたものだけを残す
    if (keep) {
        filtered_pred.push_back(box);
    }
  }

  // フィルタリング済みの結果で上書きする
  nms_pred = filtered_pred;
  
  // ==========================================
  // 【追加機能】 ここまで
  // ==========================================

  /* 元のコードにあったデバッグ出力（必要に応じてコメントアウトを外して使用） */
